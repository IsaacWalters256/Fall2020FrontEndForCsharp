class CustomerPage {
  ...
  constructor() {
    ...
    this.server = "http://localhost:5000/api" //the port that we use in the previous lab
    this.url = this.server + "/customers"; //part to append the table we are using

    // instance variables related to ui elements simplifies code in other places //stuff that gets the parts of the html application and put it into javascript variables
  ...
  fetchStates() {
    fetch(`${this.server}/states`) //gets the server we are running with host 5000 and pust the states on the end so it knows to use the states table
    .then(response => response.json())
    .then(data => { 
      if (data.length == 0) {
        alert("Can't load states.  Can not add or edit customers without state inforamtion."); //this is a promise statement where we can get the data of states
      }
      else {
    ...
    .catch(error => {
      alert('There was a problem getting customer info!');  //catch statement just in case
    });
  }
  ...
  onFindCustomer(event) {
    event.preventDefault(); //prevents the form from being submited
  ...
  makeFieldsReadOnly(readOnly=true) { //These are for changing the elements so that the user cant get confused and enter in stuff that wont do anything
